...
func ReturnRouter(config config.Config, frontendFS fs.FS) (*gin.Engine, *gorm.DB) {
	...
	v1 := router.Group("/api/v1")
	{
		...
		v1.POST("/info", middleware.AdminCheckMiddleware(), adminControllor.Info)
		//Check Admin Token
		...
		v1.POST("/key/range", middleware.AdminCheckMiddleware(), middleware.AdminPermissionCheckMiddleware("can_operate_app_key"), adminControllor.KeyRange)
...

	{
		...
		app.POST("/user/get", middleware.ApplicationKeyCheckMiddleware(), appControllor.UserGet)
		app.POST("/user/range", middleware.ApplicationKeyCheckMiddleware(), appControllor.UserRange)
		//Character Operation
		...
		app.POST("/character/get", middleware.ApplicationKeyCheckMiddleware(), appControllor.CharacterGet)
		app.POST("/character/range", middleware.ApplicationKeyCheckMiddleware(), appControllor.CharacterRange)
		//Get Metadata
